...
export const ONBOARDING_QUESTIONS: OnboardingQuestion[] = [
  { id: "q1_language", question: "Em qual idioma você prefere conversar?", type: "single",
    options: [
  ...
  { id: "q2_name", question: "Como você gostaria de ser chamado(a)?", type: "single", options: [] },
  { id: "q3_familiarity", question: "Qual sua relação com o estoicismo hoje?", type: "single",
    options: [
  ...
  { id: "q4_focus", question: "Quais áreas da vida você mais quer refletir? (escolha até 3)", type: "multi",
    options: [
  ...
  { id: "q5_dilemma", question: "Seu sócio toma uma decisão importante sem te consultar. Sua primeira reação interna costuma ser:", type: "single",
    options: [
  ...
  { id: "q6_style", question: "Numa boa conversa, você prefere:", type: "single",
    options: [
  ...
  { id: "q7_depth", question: "O que te atrai mais no estoicismo?", type: "single",
    options: [
  ...
  { id: "q8_moment", question: "Como você descreveria seu momento atual?", type: "single",
    options: [
      { value: "building", label: "Construindo algo novo", maps: { life_moment: "construção" } },
  ...
  { id: "q9_formality", question: "Prefere que eu fale com você de forma:", type: "single",
    options: [
  ...
  { id: "q10_frequency", question: "Com que frequência você imagina conversar por aqui?", type: "single",
    options: [
...
export function buildInitialProfile(answers: Record<string, string | string[]>) {
  const communicationStyle: Record<string, string> = {
    tone: "reflexivo", depth: "pratico", pace: "explorador",
    formality: "informal", emotional_expression: "medio",
  };
  const lifeContext: Record<string, unknown> = { focus_areas: [], stoic_familiarity: "iniciante" };
  ...
  for (const q of ONBOARDING_QUESTIONS) {
    ...
    for (const val of values) {
      ...
      for (const [key, mapVal] of Object.entries(option.maps)) {
        if (key === "focus_areas") {
          lifeContext.focus_areas = [...new Set([...(lifeContext.focus_areas as string[]), ...(mapVal as string[])])];
        } else if (["stoic_familiarity", "life_moment", "usage_intent", "preferred_language"].includes(key)) {
